            ...
            if fixed_pid != row[4]:
                if fixed_pid == -1:  # process the first rows
                    ...
                    if idx+1 < len(data_map[pid]):
                        ...
                        # 80 801
                        # 90 801
                        if math.isclose(row[0], data_map[pid][idx+1][0]) and data_map[pid][idx+1][4] == fixed_pid:
                    ...
                    row_buf.append(
                        {'fpid': fixed_pid, 'times': time_buf, 'plats': lat_buf,
                         'plons': lon_buf, 'angles': angle_buf})
                    fixed_pid = row[4]
                    time_buf = [row[0]]
                    lat_buf = [row[1]]
                    lon_buf = [row[2]]
