    ...
    if wrap:
        ...
        for p in resolved_polygons:
            wrapped_polygons += date_line_wrapper.wrap(
                p.get_geometry(), tesselate_degrees
            )
    ...
    for reconstructed_polygon in reconstructed_polygons:
        ...
        if lon0 is not None:
            ...
            geometries = wrapper.wrap(
                reconstructed_polygon.get_reconstructed_geometry(), tesselate_degrees
            )
    ...
    for shared_boundary_section in shared_boundary_sections:
        for shared_sub_segment in shared_boundary_section.get_shared_sub_segments():
            ...
            if lon0 is not None:
                wrapper = pygplates.DateLineWrapper(lon0)
                geometries = wrapper.wrap(
                    shared_sub_segment.get_geometry(), tesselate_degrees
                )
            else:
...
def get_json_from_reconstructed_polygons(
    ...
    for p in polygons:
        ...
        feature["geometry"]["coordinates"] = [
            list(zip(lons + lons[:1], lats + lats[:1]))
        ]  # make sure the first point is the same with the last one
        data["features"].append(feature)
