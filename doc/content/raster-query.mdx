...
the values at the given locations. return nan or null if the data is missing in the raster.

if "fmt=json" is given, for a single location, the server will return data in JSON format.

For multiple locations, the server always return data in JSON format.
