...
recon_time = 60;
js = urlread(['http://127.0.0.1:8000/reconstructions/coastlines/?time=', ...
    num2str(recon_time)]);
cs = loadjson(js);
js = urlread(['http://127.0.0.1:8000/reconstructions/static_polygons/?time=', ...
    num2str(recon_time)]);
sp = loadjson(js);
js = urlread(['http://127.0.0.1:8000/reconstructions/plate_polygons/?time=', ...
    num2str(recon_time)]);
pp = loadjson(js);
